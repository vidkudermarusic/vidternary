...
# - Chunk size: 10,000 (was 1,000) 
# - Preview samples: 10,000 (was 1,000)
# - Log entries: 10,000 (was 1,000)
# - Custom threshold warning: 10,000 (was 1,000)
required_packages <- c("openxlsx", "Ternary", "PlotTools", "shiny", "shinyFiles", 
                       "shinyjqui", "shinyBS", "ggplot2", "GGally", "rmarkdown", 
                       "corrplot", "knitr", "colourpicker", "DT", "robustbase", "isotree",
                       "plotly", "writexl", "jsonlite", "zip", "fs", "htmlwidgets", "moments")
...
generate_cache_key <- function(data_hash, filters, elements) {
  key_data <- list(
    data_hash = data_hash,
    filters = filters,
    elements = elements
...
cache_result <- function(key, result) {
  ...
  if (getOption("ternary.debug", FALSE)) {
    cat("DEBUG: Cached result for key:", substr(key, 1, 8), "...\n")
  }
}

# Function to cache plot results specifically
cache_plot_result <- function(plot_key, plot_data, plot_type = "ternary") {
  # Generate a comprehensive cache key for plots
  ...
  if (getOption("ternary.debug", FALSE)) {
    cat("DEBUG: Cached plot result for:", plot_type, "-", substr(plot_key, 1, 8), "...\n")
  }
}
...
is_plot_cached <- function(plot_key, plot_type = "ternary") {
...
get_cached_plot <- function(plot_key, plot_type = "ternary") {
  # Generate the same cache key that would be used for caching
  plot_cache_key <- digest::digest(list(
    plot_key = plot_key,
    plot_type = plot_type
...
check_and_install_packages <- function(packages) {
  ...
  if (length(missing_packages) > 0) {
    ...
    tryCatch({
      ...
      cat("Warning: Some packages could not be installed automatically.\n")
      cat("Error:", e$message, "\n")
      
      # Try alternative repositories
      tryCatch({
        cat("Trying alternative repository...\n")
        ...
        cat("Alternative installation also failed.\n")
        cat("Please install manually:\n")
        cat("install.packages(c(", 
            paste(paste0('"', missing_packages, '"'), collapse = ", "), "))\n")
        
        # Provide specific installation commands for problematic packages
        if ("robustbase" %in% missing_packages) {
          cat("\nFor robustbase, try:\n")
          cat('install.packages("robustbase", dependencies = TRUE, repos = "https://cran.rstudio.com/")\n')
        }
        if ("isotree" %in% missing_packages) {
          cat("\nFor isotree, try:\n")
          cat('install.packages("isotree", dependencies = TRUE, repos = "https://cran.rstudio.com/")\n')
        }
      })
    })
  }
  ...
  unavailable_packages <- setdiff(packages, available_packages)
  
  if (length(unavailable_packages) > 0) {
    cat("Warning: The following packages are not available:\n")
    cat(paste("  -", unavailable_packages), sep = "\n")
    cat("Some features may not work properly.\n\n")
    
    # Provide fallback options
    if ("robustbase" %in% unavailable_packages) {
      cat("Note: robustbase is required for robust Mahalanobis analysis.\n")
      cat("Consider installing from source: devtools::install_github('robustbase/robustbase')\n")
    }
    if ("isotree" %in% unavailable_packages) {
      cat("Note: isotree is required for isolation forest analysis.\n")
      cat("Consider installing from source: devtools::install_github('isotree/isotree')\n")
    }
  }
...
available_packages <- check_and_install_packages(required_packages)
...
process_data_efficiently <- function(data, operation, chunk_size = 10000) {
  ...
  for (i in 1:total_chunks) {
    ...
    # Update progress
    if (i %% 10 == 0) {
      cat(sprintf("Processed chunk %d/%d (%.1f%%)\n", i, total_chunks, (i/total_chunks)*100))
    }
  }
...
compute_mahalanobis_distance <- function(data1, data2, lambda = 1, omega = 0, keep_outliers = FALSE, custom_mdthresh = NULL, selected_columns = NULL, mdthresh_mode = "auto", chisq_confidence = 0.95) {
  # Input validation
  ...
  if (getOption("ternary.debug", FALSE)) {
    cat("DEBUG: Mahalanobis validation passed\n")
    cat("DEBUG: Variables:", validation_result$n_vars, "\n")
    cat("DEBUG: Observations (data):", validation_result$n_obs1, "\n")
    cat("DEBUG: Observations (reference):", validation_result$n_obs2, "\n")
    cat("DEBUG: Condition number:", validation_result$condition_number, "\n")
    if (!is.null(validation_result$high_correlations)) {
      cat("DEBUG: High correlations detected\n")
    }
  }
  ...
  } else if (mdthresh_mode == "chisq") {
    df <- length(common_cols)
    MDthresh <- qchisq(chisq_confidence, df = df)
    threshold_method <- sprintf("Chi-Square (%.0f%% confidence)", chisq_confidence * 100)
    threshold_formula <- sprintf("χ²(%.0f%%, df=%d) = %.3f", chisq_confidence * 100, df, MDthresh)
  } else {
...
compute_robust_mahalanobis <- function(data1, data2, method = "MCD", keep_outliers = FALSE, selected_columns = NULL) {
  ...
  # Log validation results for debugging
  if (getOption("ternary.debug", FALSE)) {
    cat("DEBUG: Robust Mahalanobis validation passed\n")
  ...
  return(list(
    ...
    outlier_99 = sum(robust_distances > threshold_99),
...
compute_isolation_forest <- function(data1, data2, contamination = 0.1, keep_outliers = FALSE, selected_columns = NULL) {
  ...
  # Log validation results for debugging
  if (getOption("ternary.debug", FALSE)) {
    cat("DEBUG: Isolation Forest validation passed\n")
    cat("DEBUG: Variables:", validation_result$n_vars, "\n")
    cat("DEBUG: Observations (data):", validation_result$n_obs1, "\n")
    cat("DEBUG: Observations (reference):", validation_result$n_obs2, "\n")
    cat("DEBUG: Condition number:", validation_result$condition_number, "\n")
...
create_distribution_plots <- function(data, output_dir = NULL, filename = "distributions") {
  ...
  cat("Distribution plots created:", length(plot_files), "files\n")
  return(plot_files)
}
...
start_performance_monitor <- function(operation_name) {
  ...
  cat("Performance monitoring started for:", operation_name, "\n")
}
...
end_performance_monitor <- function(operation_name) {
  if (!is.null(performance_monitor$operations[[operation_name]])) {
    ...
    cat(sprintf("Performance: %s completed in %.2fs, memory: %.2f MB\n", 
...
run_comprehensive_analysis <- function(data1, data2, analysis_config = NULL, output_dir = NULL) {
  ...
  # Note: Old batch_export removed - individual exports handled by generate_filtered_data_for_export()
  results$export_files <- list(
    list(
      dataset1 = data1,
      dataset2 = data2,
  ...
  write.csv(data1, results$export_files$dataset1_csv, row.names = FALSE)
  write.csv(data2, results$export_files$dataset2_csv, row.names = FALSE)
  ...
  write.csv(quality_summary_df, results$export_files$quality_summary_csv, row.names = FALSE)
...
generate_comprehensive_report <- function(results, output_dir) {
  report_html <- paste0(
    ...
    "<p>All results are saved in: ", results$output_dir, "</p>",
    "</div>",
...
general_ternary_plot <- function(
  ...
  if (use_mahalanobis) {
    ...
    if (mahalanobis_reference == "self") {
      ...
      if (!preview) cat("Using self-reference for multivariate analysis\n")
    } else if (mahalanobis_reference == "dataset1") {
      # For dataset1 reference, we need to load dataset1 data
      if (!preview) cat("Using Dataset 1 as reference for multivariate analysis\n")
      # This will be handled in the calling function
      actual_reference_data <- NULL  # Will be set by caller
    } else if (mahalanobis_reference == "dataset2") {
      actual_reference_data <- reference_data  # Dataset 2 reference
      if (!preview) cat("Using Dataset 2 as reference for multivariate analysis\n")
    }
    
    # Skip if reference dataset is not available
    if (is.null(actual_reference_data) && mahalanobis_reference != "dataset1") {
      ...
      tryCatch({
        if (use_robust_mahalanobis) {
          ...
          mahal_result <- compute_mahalanobis_distance(M, actual_reference_data, lambda, omega, keep_outliers = keep_outliers_mahalanobis, custom_mdthresh = custom_mdthresh, selected_columns = selected_columns, mdthresh_mode = mdthresh_mode, chisq_confidence = chisq_confidence)
  ...
  if (getOption("ternary.debug", FALSE)) cat("DEBUG: After multivariate filtering, data dimensions:", dim(M), "\n")
...
run_ternary_gui <- function(xlsx_file = NULL, working_dir = default_working_dir, output_dir = default_output_dir) {
  ...
  ui <- shiny::fluidPage(
    shiny::titlePanel("Ternary Plot Generator with Advanced Filtering"),
    ...
    shiny::fluidRow(
      shiny::column(12,
             shiny::div(
               ...
               shiny::actionButton("help_button", "?", 
    ...
    hr(),
    tabsetPanel(
      shiny::tabPanel("Ternary Plots",
               shiny::fluidRow(
                 shiny::column(12,
                        shiny::fluidRow(
                          shiny::column(6, h3("Dataset 1 (Primary)")),
                          shiny::column(6, h3("Dataset 2 (Reference)"))
                        ),
                        shiny::fluidRow(
                          shiny::column(6,
                                 shiny::fileInput("xlsx_file1", "Choose Primary XLSX File", accept = c(".xlsx"))
                          ),
                          shiny::column(6,
                                 shiny::fileInput("xlsx_file2", "Choose Reference XLSX File", accept = c(".xlsx"))
                          )
                        )
                 )
               ),
               ...
               shiny::fluidRow(
                 shiny::column(12,
                        ...
                        shiny::fluidRow(
                          shiny::column(6,
                                 shiny::textOutput("preview_info1"),
                                 shinyjqui::jqui_resizable(shiny::plotOutput("ternary_preview1", height = "500px"))
                          ),
                          shiny::column(6,
                                 shiny::textOutput("preview_info2"),
                                 shinyjqui::jqui_resizable(shiny::plotOutput("ternary_preview2", height = "500px"))
                          )
                        )
                 )
               ),
               ...
               # Save Plot Buttons
               shiny::fluidRow(
                 shiny::column(12, style = "text-align: center; margin: 20px 0;",
                        h4("Save Plots"),
                        shiny::actionButton("plot1", "Save Plot 1", class = "btn-primary btn-lg", style = "margin: 0 10px;"),
                        shiny::actionButton("plot2", "Save Plot 2", class = "btn-primary btn-lg", style = "margin: 0 10px;"),
                        shiny::actionButton("plot_both", "Save Both Plots", class = "btn-success btn-lg", style = "margin: 0 10px;")
                 )
               ),
               shiny::fluidRow(
                 shiny::column(12,
                        shiny::fluidRow(
                          shiny::column(6,
                                 shiny::div(style = "border: 2px solid #dc3545; padding: 10px; border-radius: 5px; margin: 10px 0;",
                                     h4(style = "color: #dc3545; margin-top: 0;", "Element A (Required)"),
                                     shiny::selectInput("element_A1", "Element A (multiple allowed):", choices = col_names, multiple = TRUE),
                                     shiny::uiOutput("dynamic_filters_A1"),
                                     shiny::helpText("Note: Each selected element can have its own filter condition (logical AND between elements)"),
                                     shiny::helpText("Example: Fe > 10, Al > 5, Si > 0 (each element gets its own threshold)")
                                 ),
                                 shiny::div(style = "border: 2px solid #dc3545; padding: 10px; border-radius: 5px; margin: 10px 0;",
                                     h4(style = "color: #dc3545; margin-top: 0;", "Element B (Required)"),
                                     shiny::selectInput("element_B1", "Element B (multiple allowed):", choices = col_names, multiple = TRUE),
                                     shiny::uiOutput("dynamic_filters_B1")
                                 ),
                                 shiny::div(style = "border: 2px solid #dc3545; padding: 10px; border-radius: 5px; margin: 10px 0;",
                                 ...
                                 shiny::selectInput("optional_param1_1", "Optional Param 1:", choices = c("", col_names)),
                                 shiny::selectInput("optional_param1_representation1", "Optional Param 1 Representation:",
                                             choices = c("Point Size" = "point_size", "Point Type" = "point_type"),
                                             selected = "point_size"),
                                 shiny::textInput("filter_op1_1", "Filter for Optional Param 1", ""),
                                 shiny::selectInput("optional_param2_1", "Optional Param 2:", choices = c("", col_names)),
                                 shiny::textInput("filter_op2_1", "Filter for Optional Param 2", ""),
                                 shiny::selectInput("color_palette1", "Color Palette for Optional Param 2:",
                          ...
                          shiny::column(6,
                                 shiny::div(style = "border: 2px solid #dc3545; padding: 10px; border-radius: 5px; margin: 10px 0;",
                                     h4(style = "color: #dc3545; margin-top: 0;", "Element A (Required)"),
                                     shiny::selectInput("element_A2", "Element A (multiple allowed):", choices = col_names, multiple = TRUE),
                                     shiny::uiOutput("dynamic_filters_A2")
                                 ),
                                 shiny::div(style = "border: 2px solid #dc3545; padding: 10px; border-radius: 5px; margin: 10px 0;",
                                     h4(style = "color: #dc3545; margin-top: 0;", "Element B (Required)"),
                                     shiny::selectInput("element_B2", "Element B (multiple allowed):", choices = col_names, multiple = TRUE),
                                     shiny::uiOutput("dynamic_filters_B2")
                                 ),
                                 shiny::div(style = "border: 2px solid #dc3545; padding: 10px; border-radius: 5px; margin: 10px 0;",
                                     h4(style = "color: #dc3545; margin-top: 0;", "Element C (Required)"),
                                     shiny::selectInput("element_C2", "Element C (multiple allowed):", choices = col_names, multiple = TRUE),
                                     shiny::uiOutput("dynamic_filters_C2")
                                 ),
                                 shiny::selectInput("optional_param1_2", "Optional Param 1:", choices = c("", col_names)),
                                 shiny::selectInput("optional_param1_representation2", "Optional Param 1 Representation:",
                                             choices = c("Point Size" = "point_size", "Point Type" = "point_type"),
                                             selected = "point_size"),
                                 shiny::textInput("filter_op1_2", "Filter for Optional Param 1", ""),
                                 shiny::selectInput("optional_param2_2", "Optional Param 2:", choices = c("", col_names)),
                                 shiny::textInput("filter_op2_2", "Filter for Optional Param 2", ""),
                                 shiny::selectInput("color_palette2", "Color Palette for Optional Param 2:",
                                             choices = c("Blue" = "blue", "Red" = "red", "Viridis" = "viridis", "Rainbow" = "rainbow"),
                                             selected = "blue")
                          )
                        )
               ...
               shiny::fluidRow(
                 shiny::column(12,
                        ...
                        shiny::fluidRow(
                          shiny::column(4,
                                 shiny::div(style = "border: 2px solid #007bff; padding: 15px; border-radius: 8px; margin: 10px 0; background-color: #f8f9fa;",
                                     h4(style = "color: #007bff; margin-top: 0;", "Multivariate Analysis"),
                                     shiny::checkboxInput("use_mahalanobis", "Use Mahalanobis Distance Filtering", value = FALSE),
                                     shiny::helpText("Standard Mahalanobis distance with custom threshold formula"),
                                     shiny::conditionalPanel(
                                       condition = "input.use_mahalanobis || input.use_robust_mahalanobis",
                                       div(style = "border: 1px solid #007bff; padding: 10px; border-radius: 5px; margin: 10px 0; background-color: #f0f8ff;",
                                           h5(style = "color: #007bff; margin-top: 0;", "📊 Reference Dataset Selection"),
                                           shiny::radioButtons("mahalanobis_reference", "Reference dataset:", 
                                                        choices = c("Dataset 2 as reference" = "dataset2", "Dataset 1 as reference" = "dataset1", "Self-reference (dataset itself)" = "self"), 
                                                        selected = "dataset2", inline = TRUE),
                                           shiny::helpText("• Dataset 2: Compare to external reference (recommended for validation)"),
                                           shiny::helpText("• Dataset 1: Use Dataset 1 as reference for Dataset 2"),
                                           shiny::helpText("• Self: Find outliers within the same dataset"),
                                           hr(style = "margin: 10px 0;"),
                                           h5(style = "color: #007bff; margin-top: 0;", "🔍 Column Selection"),
                                           shiny::selectInput("mahalanobis_columns", "Columns for Mahalanobis calculation:", 
                                                       choices = col_names, multiple = TRUE,
                                                       selected = NULL),
                                           shiny::helpText("Select specific columns for multivariate analysis"),
                                           shiny::helpText("• Leave empty to use all common numeric columns"),
                                           shiny::helpText("• Minimum 2 columns required"),
                                           shiny::helpText("• More columns = higher dimensional analysis"),
                                           shiny::actionButton("validate_columns", "Validate Column Selection", 
                                                        class = "btn-sm btn-outline-info", style = "margin-top: 5px;"),
                                           shiny::br(), shiny::br(),
                                           shiny::verbatimTextOutput("column_validation_output")
                                       )
                                     ),
                                     shiny::div(style = "border: 1px solid #dee2e6; padding: 10px; border-radius: 5px; margin: 10px 0; background-color: #ffffff;",
                                         h5("Threshold calculation:"),
                                         shiny::radioButtons("mdthresh_mode", "Threshold calculation:", 
                                                      choices = c("Automatic (MDthresh=MDmean+√(100/(100+λ-ω))×stdMD)" = "auto", "Manual value" = "manual", "Chi-Square (χ²(p, df))" = "chisq"), 
                                                      selected = "auto", inline = TRUE),
                                         shiny::conditionalPanel(
                                           condition = "input.mdthresh_mode == 'manual'",
                                           shiny::numericInput("custom_mdthresh", "Custom MDthresh value:", value = 10, min = 0, step = 0.1),
                                           shiny::helpText("Enter a positive numeric value. Higher values = more lenient (fewer outliers detected)")
                                         ),
                                         shiny::conditionalPanel(
                                           condition = "input.mdthresh_mode == 'chisq'",
                                           shiny::selectInput("chisq_confidence", "Confidence Level:", 
                                                             choices = c("90%" = 0.90, "95%" = 0.95, "99%" = 0.99), 
                                                             selected = 0.95),
                                           shiny::helpText("Statistical threshold using chi-square distribution"),
                                           shiny::helpText("95% = 2.45σ, 99% = 3.32σ (typical for outlier detection)")
                                         ),
                                         shiny::conditionalPanel(
                                           ...
                                           div(class = "info-box",
                                               h6("Threshold Formula Explanation:"),
                                               p("MDthresh = MDmean + √(100/(100+λ-ω)) × stdMD"),
                                               p("• λ (lambda): Controls threshold strictness"),
                                               p("  - Higher λ = stricter threshold = more outliers detected"),
                                               p("  - Lower λ = more lenient threshold = fewer outliers detected"),
                                               p("• ω (omega): Provides flexibility adjustment"),
                                               p("  - Higher ω = more lenient threshold"),
                                               p("  - Lower ω = stricter threshold"),
                                               p("• Typical ranges: λ = 0-10, ω = 0-5"),
                                               p("• Default: λ=1, ω=0 gives balanced threshold")
                                           )
                                         )
                                     ),
                                     ...
                                     helpText("Robust covariance estimation using Minimum Covariance Determinant. Better for data with outliers or non-normal distributions."),
                                     conditionalPanel(
                                       condition = "input.use_robust_mahalanobis",
                                       div(style = "border: 1px solid #28a745; padding: 8px; border-radius: 5px; margin: 8px 0; background-color: #f0fff0;",
                                           radioButtons("robust_mahalanobis_reference", "Reference dataset:", 
                                                        choices = c("Dataset 2 as reference" = "dataset2", "Dataset 1 as reference" = "dataset1", "Self-reference (dataset itself)" = "self"), 
                                                        selected = "dataset2", inline = TRUE),
                                           selectInput("robust_mahalanobis_columns", "Columns for Robust Mahalanobis:", 
                                                       choices = col_names, multiple = TRUE,
                                                       selected = NULL),
                                           helpText("Uses same reference and column settings as standard Mahalanobis")
                                       )
                                     ),
                                     ...
                                     helpText("Anomaly detection using isolation forest algorithm. Good for high-dimensional data and non-linear relationships."),
                                     conditionalPanel(
                                       condition = "input.use_isolation_forest",
                                       div(style = "border: 1px solid #ff8c00; padding: 8px; border-radius: 5px; margin: 8px 0; background-color: #fff8f0;",
                                           radioButtons("isolation_forest_reference", "Reference dataset:", 
                                                        choices = c("Dataset 2 as reference" = "dataset2", "Dataset 1 as reference" = "dataset1", "Self-reference (dataset itself)" = "self"), 
                                                        selected = "dataset2", inline = TRUE),
                                           selectInput("isolation_forest_columns", "Columns for Isolation Forest:", 
                                                       choices = col_names, multiple = TRUE,
                                                       selected = NULL),
                                           helpText("Uses same reference and column settings as standard Mahalanobis")
                                       )
                                 ...
                                 # Data validation status for multivariate analysis
                                 div(style = "border: 1px solid #ffc107; padding: 10px; border-radius: 5px; margin: 10px 0; background-color: #fffbf0;",
                                     h5(style = "color: #856404; margin-top: 0;", "🔍 Data Validation Status"),
                                     uiOutput("multivariate_validation_status"),
                                     actionButton("check_multivariate_readiness", "Check Data Readiness", 
                                                class = "btn-sm btn-outline-warning", style = "margin-top: 5px;"),
                                     helpText("Click to validate data suitability for multivariate analysis")
                                 )
                          ),
                          column(4,
                          ...
                          column(4,
                                 div(style = "border: 2px solid #ffc107; padding: 15px; border-radius: 8px; margin: 10px 0; background-color: #f8f9fa;",
                                     ...
                                     checkboxInput("include_plot_notes", "Include plot notes/details below plot", value = TRUE),
                                     helpText("When checked, plots will include detailed information about elements, filters, and parameters used"),
                                     div(style = "border: 1px solid #dee2e6; padding: 10px; border-radius: 5px; margin: 10px 0; background-color: #ffffff;",
                                         ...
                                         h5("Mahalanobis Distance Threshold Methods"),
                                         helpText("Choose from three threshold calculation methods:"),
                                         helpText("1. Automatic: MDthresh = MDmean + √(100/(100+λ-ω)) × stdMD (user-tunable)"),
                                         helpText("2. Custom: User-defined numeric threshold (direct control)"),
                                         helpText("3. Chi-Square: Statistical thresholds using χ²(p, df) (statistically rigorous)"),
                                         helpText("Where:"),
                                         helpText("• λ (lambda) = Scaling factor for distance sensitivity"),
                                         helpText("• ω (omega) = Additional offset threshold"),
                                         helpText("• df = Number of variables (elements) in the analysis"),
                                         helpText("• χ²(p, df) = Chi-square critical value at confidence level p")
                                     )
                                 )
                          )
                        )
      ...
      # New tab for Multiple Plot Types
      tabPanel("Multiple Plot Types",
               fluidRow(
                 column(12,
                        h3("Advanced Plotting Options"),
                        # Nested tabsetPanel for plot types
                        tabsetPanel(
                                                     tabPanel("Scatter Plots",
                             fluidRow(
                               column(4,
                                 selectizeInput("scatter_columns", "Select Columns for Analysis", 
                                               choices = NULL, multiple = TRUE),
                                 selectizeInput("scatter_x_col", "X-axis Column (optional)", 
                                               choices = NULL, multiple = FALSE),
                                 selectizeInput("scatter_y_col", "Y-axis Column (optional)", 
                                               choices = NULL, multiple = FALSE),
                             ...
                             fluidRow(
                               column(4,
                                 selectizeInput("histogram_columns", "Select Columns for Analysis", 
                                               choices = NULL, multiple = TRUE),
                             ...
                             fluidRow(
                               column(4,
                                 selectizeInput("boxplot_columns", "Select Columns for Analysis", 
                                               choices = NULL, multiple = TRUE),
                          ...
                          tabPanel("Original Options",
                        fluidRow(
                          column(6,
                                 h4("Plot Type Selection"),
                                 ...
                                 # Dataset selection
                                 radioButtons("plot_dataset", "Select Dataset:",
                                              choices = c("Dataset 1" = "dataset1", 
                                                          "Dataset 2" = "dataset2"),
                                 ...
                                 conditionalPanel(
                                   ...
                                   selectInput("scatter_x", "X-axis Column:", choices = NULL),
                                   selectInput("scatter_y", "Y-axis Column:", choices = NULL),
                                   selectInput("scatter_color", "Color by Column (optional):", choices = c("None" = "none", NULL)),
                                   numericInput("scatter_size", "Point Size:", value = 2, min = 0.5, max = 10, step = 0.5)
                                 ),
                                 
                                 conditionalPanel(
                                   condition = "input.plot_type == 'histogram'",
                                   selectInput("hist_column", "Column for Histogram:", choices = NULL),
                                   numericInput("hist_bins", "Number of Bins:", value = 30, min = 5, max = 100, step = 1),
                                   checkboxInput("hist_density", "Show Density Curve", value = TRUE)
                                 ),
                                 
                                 conditionalPanel(
                                   condition = "input.plot_type == 'correlation'",
                                   selectInput("corr_columns", "Columns for Correlation:", choices = NULL, multiple = TRUE),
                                 ...
                                 conditionalPanel(
                                   ...
                                   selectInput("box_y", "Y-axis Column:", choices = NULL),
                                   selectInput("box_x", "Group by Column (optional):", choices = c("None" = "none", NULL))
                                 ),
                                 
                                 conditionalPanel(
                                   condition = "input.plot_type == 'density'",
                                   selectInput("density_column", "Column for Density Plot:", choices = NULL),
                                   selectInput("density_group", "Group by Column (optional):", choices = c("None" = "none", NULL))
                                 ),
                                 
                                 # Sample size control
                                 numericInput("max_samples", "Maximum Number of Samples:", 
                                              value = 10000, min = 1000, max = 100000, step = 1000),
                                 helpText("Limit samples for better performance on large datasets"),
                                 
                                 actionButton("generate_plot", "Generate Plot", class = "btn-primary"),
                                 actionButton("save_advanced_plot", "Save Plot", class = "btn-success")
                          ),
                          column(6,
                                 h4("Plot Output"),
      ...
      # New tab for Multiple Ternary Creator
      tabPanel("Multiple Ternary Creator",
               fluidRow(
                 column(12,
                        h3("Create Ternary Plots for Multiple Files"),
                        helpText("This tool allows you to create ternary plots for multiple Excel files using the same parameters. Use the preview mode to test your settings before saving all plots."),
                        ...
                        fluidRow(
                          column(6,
                                 ...
                                 helpText("Select multiple Excel files to process. Each file will generate a separate ternary plot. During preview mode, plots are created but not saved to disk.")
                          ),
                          column(6,
                                 h4("Output Settings"),
                                 helpText("Output directory: Use the 'Directory Settings' section at the bottom of the app. Plots will be saved in a new subfolder. Note: This is only used when saving plots, not during preview."),
                        ...
                        fluidRow(
                          column(12,
                                 ...
                                 div(style = "border: 1px solid #dee2e6; padding: 15px; border-radius: 5px; margin: 10px 0; background-color: #f8f9fa;",
                                     ...
                                     h5("Statistical Filters"),
                                     fluidRow(
                                       column(6,
                                              checkboxInput("multiple_use_iqr_filter", "Use IQR Filter", value = FALSE),
                                              checkboxInput("multiple_keep_outliers_iqr", "Keep Outliers (IQR)", value = FALSE),
                                              checkboxInput("multiple_use_zscore_filter", "Use Z-Score Filter", value = FALSE),
                                              checkboxInput("multiple_keep_outliers_zscore", "Keep Outliers (Z-Score)", value = FALSE),
                                              checkboxInput("multiple_use_mad_filter", "Use MAD Filter", value = FALSE),
                                              checkboxInput("multiple_keep_outliers_mad", "Keep Outliers (MAD)", value = FALSE)
                                       )
                                     )
                                 )
                          )
                        ...
                        fluidRow(
                          column(12, style = "text-align: center; margin-top: 20px;",
                                 actionButton("create_multiple_ternary", "Create All Ternary Plots", 
                                            class = "btn-primary btn-lg", style = "width: 300px; font-size: 18px;"),
                                 br(), br(),
                                 actionButton("save_multiple_ternary", "Save All Plots to Subfolder", 
                                            class = "btn-success btn-lg", style = "width: 300px; font-size: 18px;")
                          )
                        ),
      ...
      column(12,
             ...
             fluidRow(
               column(12,
                      ...
                      helpText("Enable detailed debug output in console")
               )
             )
      )
    ),
  ...
  server <- function(input, output, session) {
    ...
    print_multiple_debug_info <- function(input, filters, operation = "preview") {
      cat("DEBUG", toupper(operation), ": Selected elements A:", paste(input$multiple_element_A, collapse = ", "), "\n")
      cat("DEBUG", toupper(operation), ": Selected elements B:", paste(input$multiple_element_B, collapse = ", "), "\n")
      cat("DEBUG", toupper(operation), ": Selected elements C:", paste(input$multiple_element_C, collapse = ", "), "\n")
      cat("DEBUG", toupper(operation), ": Selected optional param1:", paste(input$multiple_optional_param1, collapse = ", "), "\n")
      cat("DEBUG", toupper(operation), ": Selected optional param2:", paste(input$multiple_optional_param2, collapse = ", "), "\n")
      
      cat("DEBUG", toupper(operation), ": Collected filters A:", if(length(filters$individual_filters_A) > 0) paste(names(filters$individual_filters_A), "=", unlist(filters$individual_filters_A), collapse = ", ") else "none", "\n")
      cat("DEBUG", toupper(operation), ": Collected filters B:", if(length(filters$individual_filters_B) > 0) paste(names(filters$individual_filters_B), "=", unlist(filters$individual_filters_B), collapse = ", ") else "none", "\n")
              cat("DEBUG", toupper(operation), ": Collected filters C:", if(length(filters$individual_filters_C) > 0) paste(names(filters$individual_filters_C), "=", unlist(filters$individual_filters_C), collapse = ", ") else "none", "\n")
      cat("DEBUG", toupper(operation), ": Collected optional param1 filters:", if(length(filters$optional_param1_filters) > 0) paste(names(filters$optional_param1_filters), "=", unlist(filters$optional_param1_filters), collapse = ", ") else "none", "\n")
      cat("DEBUG", toupper(operation), ": Collected optional param2 filters:", if(length(filters$optional_param2_filters) > 0) paste(names(filters$optional_param2_filters), "=", unlist(filters$optional_param2_filters), collapse = ", ") else "none", "\n")
    }
    ...
    create_scatter_plots <- function(data, columns, colors = NULL, x_column = NULL, y_column = NULL, point_size = 0.8) {
      if (is.null(colors)) {
      ...
      plot(data[[x_column]], data[[y_column]], 
           ...
           xlab = x_column, 
           ylab = y_column,
           main = "Scatter Plot with Multiple Columns")
      ...
      return(list(x_column = x_column, y_column = y_column, colors = colors))
    }
    ...
    create_histograms <- function(data, columns, colors = NULL, breaks = "Sturges", alpha = 0.7, 
                                 manual_breaks = NULL, data_type = "frequency") {
      if (is.null(colors)) {
        colors <- rainbow(length(columns))
      }
      ...
      plot(hist_data, 
           ...
           xlab = "Value",
           ylab = y_lab,
           main = "Histograms with Multiple Columns")
    ...
    create_box_plots <- function(data, columns, colors = NULL, horizontal = FALSE, 
                                box_width = 0.8, notch = FALSE, show_outliers = TRUE) {
      if (is.null(colors)) {
        colors <- rainbow(length(columns))
      }
      ...
      boxplot(plot_data, 
              ...
              main = "Box Plots with Multiple Columns")
      ...
      return(list(colors = colors, horizontal = horizontal, box_width = box_width))
    }
    ...
    observeEvent(input$xlsx_file1, {
      ...
      updateSelectInput(session, "robust_mahalanobis_columns", choices = numeric_cols, selected = character(0))
      updateSelectInput(session, "isolation_forest_columns", choices = numeric_cols, selected = character(0))
    })
    ...
    observeEvent(input$xlsx_file2, {
      ...
      if (!is.null(input$xlsx_file1)) {
        ...
        # Update all Mahalanobis-related column inputs
        updateSelectInput(session, "mahalanobis_columns", choices = common_numeric_cols, selected = character(0))
        updateSelectInput(session, "robust_mahalanobis_columns", choices = common_numeric_cols, selected = character(0))
        updateSelectInput(session, "isolation_forest_columns", choices = common_numeric_cols, selected = character(0))
      }
    })
    ...
    # Column validation button observer
    observeEvent(input$validate_columns, {
      ...
      selected_cols <- input$mahalanobis_columns
      reference_mode <- input$mahalanobis_reference
      
      validation_result <- validate_mahalanobis_columns(df1, df2, selected_cols, reference_mode)
      
      output$column_validation_output <- renderPrint({
        cat(validation_result$message)
      })
      
      # Show user message
      if (validation_result$valid) {
        show_message("Column validation successful!", "success")
      } else {
        show_message("Column validation failed. Check the output for details.", "error")
      }
    })
    ...
    observeEvent(input$save_scatter, {
      ...
      tryCatch({
        # Get the current plot from the output
        plot_obj <- output$scatter_plot_output
    ...
    observeEvent(input$save_histogram, {
      ...
      tryCatch({
        # Get the current plot from the output
        plot_obj <- output$histogram_plot_output
    ...
    observeEvent(input$save_boxplot, {
      ...
      tryCatch({
        # Get the current plot from the output
        plot_obj <- output$boxplot_plot_output
    ...
    multivariate_analysis <- reactive({
      ...
      tryCatch({
        ...
        if (input$use_robust_mahalanobis) {
          ...
          result <- compute_mahalanobis_distance(data_to_analyze, reference_data, input$lambda, input$omega, custom_mdthresh = custom_mdthresh_val, selected_columns = selected_cols, mdthresh_mode = input$mdthresh_mode, chisq_confidence = input$chisq_confidence)
        }
    ...
    output$mahalanobis_info <- renderPrint({
      ...
      if (is.null(result)) {
        ...
        if (input$use_robust_mahalanobis) {
          ...
          } else if (input$mdthresh_mode == "chisq") {
            cat("Threshold mode: Chi-Square (p =", input$chisq_confidence, ")\n")
        ...
        if (input$use_robust_mahalanobis) {
          ...
          cat("- Uses chi-square distribution for threshold (95% = 2.45σ, 99% = 3.32σ)\n")
          ...
          cat("- Compare custom threshold with chi-square thresholds (95%, 99%)\n")
        }
      }
    })
    ...
    output$ternary_preview1 <- renderPlot({
      ...
      general_ternary_plot(
        ...
        chisq_confidence = input$chisq_confidence,
        # Inclusion area feature removed
    ...
    output$ternary_preview2 <- renderPlot({
      ...
      if ((input$use_mahalanobis || input$use_robust_mahalanobis || input$use_isolation_forest) && !is.null(input$xlsx_file1)) {
        # Use proper reference based on mahalanobis_reference setting
        if (input$mahalanobis_reference == "self") {
          # Self-reference: use dataset2 as both data and reference
          reference_data <- get_cached_data(input$xlsx_file2$datapath, "dataset2")
        } else if (input$mahalanobis_reference == "dataset1") {
          # Dataset1 reference: use dataset1 as reference for dataset2
          reference_data <- get_cached_data(input$xlsx_file1$datapath, "dataset1")
        } else if (input$mahalanobis_reference == "dataset2") {
          # Dataset2 reference: use dataset2 as reference for dataset1
          reference_data <- get_cached_data(input$xlsx_file2$datapath, "dataset2")
        }
      }
      ...
      result <- safe_execute({
        withProgress(message = "Generating Plot 1", value = 0, {
          ...
          if ((input$use_mahalanobis || input$use_robust_mahalanobis || input$use_isolation_forest) && !is.null(input$xlsx_file2)) {
            ...
            # Use proper reference based on mahalanobis_reference setting
            if (input$mahalanobis_reference == "self") {
              # Self-reference: use dataset1 as both data and reference
              reference_data <- get_cached_data(input$xlsx_file1$datapath, "dataset1")
            } else if (input$mahalanobis_reference == "dataset1") {
              # Dataset1 reference: use dataset1 as reference for dataset1 (same as self)
              reference_data <- get_cached_data(input$xlsx_file1$datapath, "dataset1")
            } else if (input$mahalanobis_reference == "dataset2") {
              # Dataset2 reference: use dataset2 as reference for dataset1
              reference_data <- get_cached_data(input$xlsx_file2$datapath, "dataset2")
    ...
    observeEvent(input$plot2, {
      ...
      result <- safe_execute({
        withProgress(message = "Generating Plot 2", value = 0, {
          ...
          if ((input$use_mahalanobis || input$use_robust_mahalanobis || input$use_isolation_forest) && !is.null(input$xlsx_file1)) {
            setProgress(0.15, detail = "Loading reference data...")
            # Fix: Use proper reference based on mahalanobis_reference setting
            if (input$mahalanobis_reference == "self") {
              reference_data <- get_cached_data(input$xlsx_file2$datapath, "dataset2")
            } else {
              reference_data <- get_cached_data(input$xlsx_file1$datapath, "dataset1")
    ...
    observeEvent(input$help_button, {
      showModal(modalDialog(
        ...
        tagList(
          ...
          tags$ul(
            tags$li(strong("95% threshold:"), "Chi-square distribution (2.45σ)"),
            tags$li(strong("99% threshold:"), "Chi-square distribution (3.32σ)"),
        ...
        for (i in seq_along(input$multiple_xlsx_files$datapath)) {
          ...
          tryCatch({
            general_ternary_plot(
              ...
              use_iqr_filter = input$multiple_use_iqr_filter,
              use_zscore_filter = input$multiple_use_zscore_filter,
              use_mad_filter = input$multiple_use_mad_filter,
              lambda = 1,  # Default value since UI removed
              omega = 0,   # Default value since UI removed
              keep_outliers_mahalanobis = FALSE,  # Disabled since UI removed
              keep_outliers_robust = FALSE,       # Disabled since UI removed
              keep_outliers_isolation = FALSE,    # Disabled since UI removed
              keep_outliers_iqr = input$multiple_keep_outliers_iqr,
              keep_outliers_zscore = input$multiple_keep_outliers_zscore,
              keep_outliers_mad = input$multiple_keep_outliers_mad,
    ...
    # Multivariate analysis validation status
    output$multivariate_validation_status <- renderUI({
      if (is.null(input$xlsx_file1) || is.null(input$xlsx_file2)) {
        return(div(
          style = "color: #856404; font-weight: bold;",
          "⚠️ Upload both datasets to check validation status"
        ))
      }
      
      # Check if any multivariate method is selected
      if (!(input$use_mahalanobis || input$use_robust_mahalanobis || input$use_isolation_forest)) {
        return(div(
          style = "color: #6c757d; font-style: italic;",
          "ℹ️ Select a multivariate analysis method to check data readiness"
        ))
      }
      ...
      div(
        style = "color: #856404; font-weight: bold;",
        "🔍 Click 'Check Data Readiness' to validate data suitability"
      )
    })
    
    # Check multivariate data readiness
    observeEvent(input$check_multivariate_readiness, {
      req(input$xlsx_file1, input$xlsx_file2)
      
      show_message("Checking data readiness for multivariate analysis...", "info")
      ...
      tryCatch({
        ...
        selected_cols <- NULL
        if (input$use_mahalanobis && length(input$mahalanobis_columns) > 0) {
          selected_cols <- input$mahalanobis_columns
        } else if (input$use_robust_mahalanobis && length(input$robust_mahalanobis_columns) > 0) {
          selected_cols <- input$robust_mahalanobis_columns
        } else if (input$use_isolation_forest && length(input$isolation_forest_columns) > 0) {
          selected_cols <- input$isolation_forest_columns
        }
        
        # Determine which method to validate
        method_name <- "General Multivariate Analysis"
        min_obs_ratio <- 2
        
        if (input$use_robust_mahalanobis) {
          method_name <- "Robust Mahalanobis (MCD)"
          min_obs_ratio <- 3
        } else if (input$use_isolation_forest) {
          method_name <- "Isolation Forest"
          min_obs_ratio <- 1.5
        } else if (input$use_mahalanobis) {
          method_name <- "Mahalanobis Distance"
          min_obs_ratio <- 2
        }
        
        # Run validation
        validation_result <- validate_multivariate_data(data1, data2, selected_cols, method_name, min_obs_ratio)
        ...
        output$multivariate_validation_status <- renderUI({
          div(
            ...
            br(),
            "📊 Variables:", validation_result$n_vars,
            br(),
            "📈 Observations (Dataset 1):", validation_result$n_obs1,
            br(),
            "📈 Observations (Dataset 2):", validation_result$n_obs2,
            br(),
            "🔢 Condition number:", sprintf("%.2e", validation_result$condition_number),
            if (!is.null(validation_result$high_correlations)) {
              list(
                br(),
                "⚠️ High correlations detected (may affect numerical stability)"
              )
            }
          )
        })
        
        show_message("Data validation completed successfully!", "success")
        ...
        show_message(paste("Data validation failed:", e$message), "error")
      })
    })
    ...
    observeEvent(input$export_all, {
      ...
      show_message("Exporting all data...", "info")
      
      # Export filtered data
      if (input$export_filtered_data) {
        tryCatch({
          # Generate filtered data based on current filter configuration
          df <- generate_filtered_data_for_export(1)  # Dataset 1
          
          if (input$export_format == "xlsx") {
            filename <- paste0("filtered_data_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".xlsx")
            openxlsx::write.xlsx(df, file.path(if (length(working_dir()) > 0) working_dir() else getwd(), filename))
          } else if (input$export_format == "csv") {
            filename <- paste0("filtered_data_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".csv")
            write.csv(df, file.path(if (length(working_dir()) > 0) working_dir() else getwd(), filename), row.names = FALSE)
          } else if (input$export_format == "rds") {
            filename <- paste0("filtered_data_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".rds")
            saveRDS(df, file.path(if (length(working_dir()) > 0) working_dir() else getwd(), filename))
          }
          
          show_message(paste("Filtered data exported to:", filename), "success")
        }, error = function(e) {
          show_message(paste("Export error:", e$message), "error")
        })
      }
      ...
      if (input$export_stats) {
        tryCatch({
          if (exists("rv$stats_results") && !is.null(rv$stats_results)) {
            filename <- paste0("statistics_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".xlsx")
            openxlsx::write.xlsx(rv$stats_results, file.path(if (length(working_dir()) > 0) working_dir() else getwd(), filename))
            show_message(paste("Statistics exported to:", filename), "success")
          }
        }, error = function(e) {
      ...
      if (input$export_plots) {
        show_message("Plot export functionality will be implemented in the next version", "info")
      }
      
      # Export project
      if (input$export_project) {
        show_message("Project export functionality will be implemented in the next version", "info")
      }
    })
  }
